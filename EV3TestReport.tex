...
Plug and play functionality; (once the Eclipse plugin was installed) I was able to push code to the block with a single button press.
 		\subsubsection{Windows}
Plug and play functionality. Drivers already present in Windows 7 and Windows 8 but need to be installed manually. Install device as a Microsoft Corporation: Remote NDIS Compatible Device by going to device manager and manually selecting the driver from a list of drivers. Once driver is installed, the device will appear as an additional Ethernet LAN connection. You can then either allow DHCP to assign an IPv4 address or manually assign an IPv4 address. Once IP address is obtained, follow the instructions in step 12 of Section 3.1.1 to incorporate one-click uploading for Eclipse. 
...
Increased sensitivity or functionality in all sensors greatly increases accuracy and stability of the robots functions. The changes in the API makes coding more intuitive with less code structural decisions that have to be 'worked around'.\\ 
  ...
  \small 0.9 & \small 19 Dec 14 & \small Thomas McAtee & \small Proof read document and edited certain sections \\ \hline 
  \small 0.91 & \small 18 Dec 14 & \small Xiang Guo & \small Added USB functionality for Windows description. \\ \hline 
