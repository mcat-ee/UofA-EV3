...
The Escape button has been relocated to directly underneath the LCD display slightly shifted to the left. Functionality remains the same as the NXT Escape button. \\
The new button scheme can be observed in figure 2. 
  	...
  	\subsection{Sensors}
		...
		\begin{figure}[H]
		\centering
		\includegraphics[scale=0.25]{img/NXTLightSensor.png}
		\caption{NXT Light Sensor}
		\end{figure}
		...
		\begin{figure}[H]
		\centering
		\includegraphics[scale=0.25]{img/EV3ColorSensor.png}
		...
		\end{figure}
  		...
  		The color mode is meant to differentiate between up to 8 different colors, black, gray, red, green, yellow, blue, magenta, white. Testing was done using a laser printed standard 80gsm A3 paper, which contained a 256 color RGB chart. All tests were conducted at a range of approximately 5mm and at varying angles between 60 to 120 degrees. The sensor was able to pick up solid primaryÄù colors without issue at the angles tested. By primary we mean any colour where it's RGB hex value was represented by only pairs of F or 0. Eg. FFFFFF for white, FF0000 for red, FFFF00 for yellow etc. \\

  		Intermediate colors produced mixed results depending on the angle. Eg. FF8700 can be recognised as red at certain angles and as yellow at other angles. This is due to the reflection of the floodlight used by the sensor and the reflectivity of the ink on the paper. Therefore it is recommended in future projects to use only primaryÄ colors and non reflective paper to reduce the likelihood of erroneous readings. Other precautions that can be taken is to use colors that a true primary RGB e.g. Black 000000, white, FFFFFF, Red FF0000, Green 00FF00 or Blue 0000FF only and calibrate the sensor to only return one of those options. \\
	...
	\subsection{Building blocks}
		...
		\subsubsection{Spare pieces}
		The supplied kits came with all parts mentioned on the check list plus one of each of the following parts as spares.
		\begin{figure}[H]
		\begin{subfigure}{.24\textwidth}
		  \centering
		  \includegraphics[scale=0.23]{img/Piece1.jpg}
		  \caption{Part 4239601}
		\end{subfigure}%
		\begin{subfigure}{.24\textwidth}
		  \centering
		  \includegraphics[scale=0.9]{img/Piece2.jpg}
		  \caption{Part 4539880}
		\end{subfigure}
		\begin{subfigure}{.24\textwidth}
		  \centering
		  \includegraphics[scale=0.9]{img/Piece3.jpg}
		  \caption{Part 4565452}
		\end{subfigure}		
		\begin{subfigure}{.24\textwidth}
		  \centering
		  \includegraphics[scale=0.2]{img/Piece4.jpg}
		  \caption{Part 4211807}
		\end{subfigure}		
		\end{figure}	
	...
	\subsubsection{Installation}
		...
		\item Go to Help then Install New Software on the top menu bar
		...
		\item Give the respository a name such as "EV3 LeJOS". Then enter the URL \url{http://lejos.sourceforge.net/tools/eclipse/plugin/ev3} in the Location test box and click OK.
		...
		\item In a few seconds "leJOS EV3 Support" will appear in the selection box. Check the tick box next to this and click Next. Eclpse will now calculate the requirements and dependencies for the plugin, this step can take a minute or so. 
		\begin{figure}[H]
	  	\centering
...
The change from the use of a proprietary operating system to a Unix-based one has also enabled greater connectivity; EV3 blocks can obtain and hold IP addresses which make them incredibly easy to address and connect to. Additionally the devices can also be SSHed directly to enabling very deep configurability of the OS and robot.
  ...
  \small 0.7 & \small 18 Dec 14 & \small Thomas McAtee & \small Formalised button section. \\ \hline 
  \small 0.8 & \small 18 Dec 14 & \small Xiang Guo & \small Added to section 2.1.4, 2.2 and 3.1 \\ \hline
  \small 0.81 & \small 18 Dec 14 & \small Xiang Guo & \small Added section 2.4.4 Spare parts  \\ \hline
